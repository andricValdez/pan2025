...
def save_model(model, path_weights="gnn_model.pth", path_config="gnn_config.json"):
    ...
    config = {
        ...
        "output_dim": model.post_mp[-1].out_features,     
        ...
        "post_mp_layers": model.post_mp_layers, 
...
def extract_graph_feat(
        set_texts, set_labels, project_after_concat, add_pos_feat, graph_type,
    ...
    data_list = extract_feat(graph_data, set_texts, set_labels, device, tokenizer, 
...
def test_inference(model_save_path, model_config_path,
                   tokenizer, lang_model, output_dir, device,
                   cut_off_dataset='test_cutoff_100', batch_size=32, 
    ...
    _, _, test_text_set = utils.read_pan25_dataset(print_info=False)
    ...
    print(f"\n----> Test-Loss: {test_loss:.4f} | Test-Acc: {test_accuracy:.4f} | Test-F1Macro: {test_f1_macro:.4f}")
...
def main(dataset_name, cut_off_dataset, cuda_num=0, 
    ...
    if build_graph: 
        ...
        if dataset_name == 'pan25':
            train_text_set, val_text_set, _ = utils.read_pan25_dataset(print_info=True)
        if dataset_name == 'autext23':
            train_text_set, val_text_set, _ = utils.read_autext_dataset(print_info=False)
        ...
        print("distro_train_val_test: ", len(train_set), len(val_set))
        print("label_distro_train_val_test: ", train_set.value_counts('label'), val_set.value_counts('label'))
        ...
        graph_train, data_train_list = extract_graph_feat(train_texts, train_labels, project_after_concat, add_pos_feat, graph_type, input_proj_dim,
            pos_emb_dim, dep_emb_dim, reduced_dim, reduce_dim_emb, tokenizer, lang_model, file_name_data, output_dir, device, partition='train')
        graph_val, data_val_list = extract_graph_feat(val_texts, val_labels, project_after_concat, add_pos_feat, graph_type, input_proj_dim,
            pos_emb_dim, dep_emb_dim, reduced_dim, reduce_dim_emb, tokenizer, lang_model, file_name_data, output_dir, device, partition='val')
    ...
    for epoch in range(num_epochs):
        ...
        print(f"Epoch {epoch:02d} | Train-Loss {train_loss:4f}  | Val-Loss {val_loss:4f} | Val-Acc: {val_accuracy:4f} | Val-F1: {val_f1_macro:4f}")
    ...
    mlflow.log_metric(key=f"Best-Accuracy-val", value=float(best_val_acc_score))
    mlflow.log_metric(key=f"Best-F1Macro-val", value=float(best_val_f1_score))
    ...
    limit = None
    test_text = list(test_set['text'])[:limit]
    test_labels = list(test_set['label'])[:limit]

    graph_test, data_test_list = extract_graph_feat(test_text, test_labels, project_after_concat, add_pos_feat, graph_type, input_proj_dim,
            pos_emb_dim, dep_emb_dim, reduced_dim, reduce_dim_emb, tokenizer, lang_model, file_name_data, output_dir, device, partition='test')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test_inference", action="store_true", help="Run test inference only")
